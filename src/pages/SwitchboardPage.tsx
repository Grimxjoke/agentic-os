...
const nodeData = {
  ...
  workspace: { label: "Workspace", detail: "142 fichiers", icon: FolderTree, tone: "cyan" },
  ...
  vibe: { label: "Vibe-Trading", detail: "Déconnecté", icon: Zap, tone: "amber" },
...
export function SwitchboardPage() {
  ...
  return <div className="page switchboard-page">
    <PageHeader eyebrow="Shared infrastructure map" title="System Switchboard" description="Voyez immédiatement quels agents, services et espaces partagent du contexte — et simulez leurs connexions sans terminal." actions={<><span className="switchboard-health"><CheckCircle2 size={13} />{activeCount}/{connections.length} liens actifs</span><button className="button primary" onClick={test}><Radio size={14} />{testing ? "Test des signaux…" : "Tester les connexions"}</button></>} />

    <div className="switchboard-layout reveal delay-1">
      <section className="switchboard-canvas glass-panel">
        <header><div><p className="section-kicker">Topology · localhost</p><h3>Control plane partagé</h3></div><div className="switchboard-legend"><span><i className="active" />Actif</span><span><i />Simulé</span></div></header>
        <div className={"network-stage " + (testing ? "testing" : "")}>
          <svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Connexions entre Pi, Codex, le VPS partagé et les services">
          ...
          {testing && <div className="signal-scan"><Radio size={18} /><span>Propagation des signaux simulés</span></div>}
        </div>
        <footer><span><ShieldCheck size={14} />Aucun secret ou trafic réel n’est exposé dans cette vue.</span><button className="text-button" onClick={() => setConnections(seedConnections)}><RefreshCw size={13} />Réinitialiser la topologie</button></footer>
      </section>
      ...
      <aside className="switchboard-inspector glass-panel">
        ...
        <section><p className="detail-label">Contrôles rapides</p><div className="switchboard-actions"><button className="button secondary"><Play size={13} />Ping</button><button className="button secondary"><Database size={13} />Inspecter</button><button className="button secondary"><Cloud size={13} />Monter</button><button className="button secondary"><CircleOff size={13} />Isoler</button></div></section>
        <div className="shared-context-proof"><Sparkles size={16} /><div><strong>Contexte partagé</strong><p>Pi et Codex voient la même version des fichiers, skills, agents et missions.</p></div></div>
      </aside>
    </div>

    <section className="switchboard-services reveal delay-2">
      {Object.entries(nodeData).filter(([id]) => !["pi", "codex"].includes(id)).map(([id, node]) => { const Icon = node.icon; const online = connections.some((connection) => connection.active && (connection.from === id || connection.to === id)); return <article key={id}><span className={"service-dot " + (online ? "online" : "")}><Icon size={15} /></span><div><strong>{node.label}</strong><small>{node.detail}</small></div><em>{online ? "Reachable" : "Standby"}</em><button className="icon-button" aria-label={"Configurer " + node.label}><Cpu size={14} /></button></article>; })}
    </section>
  </div>;
}
